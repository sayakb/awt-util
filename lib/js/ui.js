...
var ui = {
	...
	*/
	loadProfileViewer: function() {
		...
		// Handler for test metadata button
		$('#profile-meta')
			.off('click')
			.on('click', function() {
				popups.showMetadataModal(profiles.data.meta);
			});
	...
	*/
	loadPopupElements: function() {
		...
		// Handle save button for metadata modal
		$('#popup-meta-save')
			.off('click')
			.on('click', function() {
				profiles.data.meta = [];

				$('.test-meta').each(function(i, item) {
					var itemVal = $(this).val().trim();

					if (itemVal.length > 0) {
						profiles.data.meta.push(itemVal);
					}
				});

				$('#popup-meta').modal('hide');
				profiles.sync();
			});
		...
		$('#popup-test-save')
			...
			.on('click', function() {
				...
				var parentId = parent != null && parent.length > 0 ? parseInt(parent) : 0;

...
var validator = {
	...
	*/
	check: function(id) {
		...
		urlParsed = test.url;
		urlParsed = validator.parseMeta(urlParsed);
		urlParsed = validator.parseDeps(urlParsed);
		...
		$.each(requestParsed, function(i) {
			requestParsed[i] = validator.parseMeta(requestParsed[i]);
			requestParsed[i] = validator.parseDeps(requestParsed[i]);
		});
		...
		$.ajax({
			...
			$.each(test.response, function(i, resp) {
				...
				respParsed = validator.parseMeta(respParsed);
				respParsed = validator.parseDeps(respParsed);
	...
	* @return string
	*/
	parseMeta: function(value) {
		var ref = value.indexOf('$meta');

		if (ref != -1) {
			var reference = value.substring(ref, 6);
			...
			if (!isNaN(metaIdx) && metaIdx >= 1 && metaIdx <= 8) {
				...
				if (metaValue !== undefined) {
					return value.replace(reference, metaValue);
				}
			}
		}

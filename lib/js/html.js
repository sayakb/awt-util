...
var html = {
	...
	*/
	...
	/**
	* Creates a link for the test status
	*
	* @param statis	Test status
	* @return string
	*/
	createTestStatusText: function(id, status) {
		switch(status) {
			case constants.validator.passed:
			case constants.validator.failed:
			case constants.validator.stalled:
				return '<a href="#" onclick="popups.showResponseModal(' + id + ')">' + status + '</a>';

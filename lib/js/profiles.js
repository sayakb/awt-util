	...
	*/
	sync: function() {
		if (profiles.name != null) {
			var data = $.extend(true, {}, profiles.data);
			data = profiles.escape(data);

			return profiles.save(profiles.name, $.stringify(data), true);
		}
		else {
	...
	*/
	createTest: function(id, name, url, parent, method) {
		...
		profiles.data.testCases.push({
			...
			parent: parent,
			method: method,
			request: $.extend(true, {}, profiles.request),
			response: $.extend(true, [], profiles.response)
		});
	},
